package com.han.gulimall.seckill.service.impl;
...
import com.han.gulimall.seckill.feign.CouponFeignService;
import com.han.gulimall.seckill.feign.ProductFeignService;
import com.han.gulimall.seckill.interceptor.LoginUserInterceptor;
import com.han.gulimall.seckill.service.SeckillService;
import com.han.gulimall.seckill.to.SeckillSkuRedisTo;
import com.han.gulimall.seckill.vo.SeckillSessionWithSkusVo;
import com.han.gulimall.seckill.vo.SkuInfoVo;
...
public class SeckillServiceImpl implements SeckillService {
    ...
    private final String SECKILL_USER_SUCCESS = "seckill:user:success:";
    ...
    private void saveSessionInfos(List<SeckillSessionWithSkusVo> sessions) {
        ...
        sessions.stream().forEach(session -> {
            ...
            /**
             * 幂等性处理，判断redis有没有商品信息，没有才进行添加上架秒杀商品。
             */
            ...
            if (!hasKey) {
                ...
                log.info("秒杀活动场次添加到redis中-->key:[{}]；value:[{}]",key,skuIds);
    ...
    private void saveSessionSkuInfo(List<SeckillSessionWithSkusVo> sessions) {
        ...
        sessions.stream().forEach(session -> {
            ...
            session.getRelationSkus().stream().forEach(seckillSkuVo -> {
                ...
                /**
                 * 定义redis存储秒杀商品的key   构成：  活动场次ID + 商品ID
                 */
                String redisKey = seckillSkuVo.getPromotionSessionId().toString() + "-" + seckillSkuVo.getSkuId().toString();
                /**
                 * 幂等性处理，先查询redis 中是否已经存在这个key，存在就表示这个商品已经配上架到秒杀中
                 */
                if (!operations.hasKey(redisKey)) {
                    ...
                    log.info("上架秒杀的商品成功key:[{}]--->value:活动ID-商品ID：{}",SECKILL_CHARE_PREFIX,redisKey);
...
//        try (Entry entry = SphU.entry("seckillSkus")) {
            ...
            for (String key : keys) {
                ...
                String replace = key.replace(SESSION__CACHE_PREFIX, "");// 前缀替换为空，就得到了开始时间和结束时间的字符串。
                String[] s = replace.split("_"); // 将时间字符串分割，得到开始时间和结束时间
                //获取存入Redis商品的开始时间
...
//    }
    ...
    public String kill(String killId, String key, Integer num) {
        ...
        if (currentTime >= startTime && currentTime <= endTime) {
            ...
            /**
             *  验证随机码是否正确，只有秒杀开始后，进入秒杀商品，点击秒杀，才会携带正确的随机码
             *
             *      如果是个人伪造的秒杀请求，由于随机码不正确，就不能秒杀商品
             */
            if (randomCode.equals(key) && killId.equals(skuId)) {
                ...
                /**
                 * 获取信号量，就是判断秒杀商品是否已经被抢完，信号量就是秒杀商品的秒杀数量
                 */
                if (count > 0 && num <= seckillLimit && count > num ) {
                    //4、验证这个人是否已经买过了（幂等性处理）,如果秒杀成功，就去占位。userId-sessionId-skuId
                    //SETNX 原子性处理
                    String redisKey = SECKILL_USER_SUCCESS + user.getId() + "-" + skuId;
                    //设置自动过期(活动结束时间-当前时间)
                    Long ttl = endTime - currentTime;
                    /**
                     * 防止用户多次抢购，只能抢一个，和抢一次
                     *
                     *  用户只要抢到了 秒杀商品，就将用户的信息存储在redis中。过期时间为直到活动结束。 这样就可以防止用户多抢
                     */
                    ...
                    if (aBoolean) {
                         ...
                         * 尝试获取信号量，等待100微妙
                         */
                        boolean semaphoreCount = semaphore.tryAcquire(num); // 不等待
//                        boolean semaphoreCount = semaphore.tryAcquire(num, 100, TimeUnit.MILLISECONDS);
                        ...
                        if (semaphoreCount) {
                            ...
                            /**
                             * 秒杀成功，发信息给MQ，异步处理秒杀成功的商品。order服务模块有监听这个队列的监听器
                             */
